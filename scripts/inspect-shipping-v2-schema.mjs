...
async function main() {
  ...
  const validation = validateExpectedItems(schema.tables[TABLES.items]);
  if (!validation.ok) {
    console.error("Validación Shipping Items falló.");
    console.error("Campos encontrados:");
    console.error(validation.found.map((field) => `- ${field}`).join("\n"));
    console.error("Campos faltantes:");
    console.error(validation.missing.map((field) => `- ${field}`).join("\n"));
    console.error("Campos parecidos posibles:");
    for (const [field, similar] of Object.entries(validation.similar)) {
  ...
  for (const field of validation.found) console.log(`- ${field}`);
}
