...
// puede representar varias unidades y venderse en partes.
{
  const bloqueo = evaluarItemNoListo(
    ...
    { reservado: false, tieneFacturaPrevia: false, cantidad: 1 , cantidadReservada: 0}
  );
...
}
{
  ...
  const bloqueo = evaluarItemNoListo(
    ...
    { reservado: true, tieneFacturaPrevia: true, cantidad: 0 , cantidadReservada: 0}
  );
...
}
{
  const bloqueo = evaluarItemNoListo(
    ...
    { reservado: true, tieneFacturaPrevia: false, cantidad: 1 , cantidadReservada: 0}
  );
...
}
{
  ...
  const bloqueo = evaluarItemNoListo(
    ...
    { reservado: false, tieneFacturaPrevia: true, cantidad: 0 , cantidadReservada: 0}
  );
...
}
{
  ...
  const bloqueo = evaluarItemNoListo(
    ...
    { reservado: true, tieneFacturaPrevia: false, cantidad: 0 , cantidadReservada: 0}
  );
...
}
{
  ...
  const bloqueo = evaluarItemNoListo(
    ...
    { reservado: true, tieneFacturaPrevia: true, cantidad: 3 , cantidadReservada: 0}
  );
...
{
  // F-42 — registro multiunidad parcialmente apartado. Caso REP-000017: 52
  // unidades, 1 comprometida. La bandera `reservado` está APAGADA a propósito
  // (solo se enciende al agotarse las libres), pero el artículo sí está
  // apartado y debe poder facturarse. Sin esta rama, arreglar F-42 habría
  // dejado sin facturar todo el stock multiunidad.
  const bloqueo = evaluarItemNoListo(
    { id: "recMU", nombre: "RAM DDR4 (1 de 52 apartada)", precio: 25 },
    { reservado: false, tieneFacturaPrevia: false, cantidad: 52, cantidadReservada: 1 }
  );
  assert(bloqueo === null, "Multiunidad con 1 unidad apartada NO debe bloquear aunque la bandera esté apagada");
}
{
  // Y el contrario sigue bloqueando: nada apartado, por ninguna de las dos vías.
  const bloqueo = evaluarItemNoListo(
    { id: "recMU0", nombre: "RAM DDR4 (nada apartado)", precio: 25 },
    { reservado: false, tieneFacturaPrevia: false, cantidad: 52, cantidadReservada: 0 }
  );
  assert(bloqueo?.motivo === "NO_RESERVADO", "Multiunidad sin ninguna unidad apartada sigue bloqueando");
}
