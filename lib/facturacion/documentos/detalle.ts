...
export async function obtenerCuerpoDocumento(tipo: TipoDocumento, recordId: string): Promise<DocumentoCuerpo | null> {
  if (tipo === "factura") {
    ...
    return { mostrarIva: true, items, subtotal: f.subtotal, iva: f.iva, total: f.total, formaPago, nota: "", motivo: "", validezDias: null };
  }
  ...
  return { mostrarIva: true, items, subtotal, iva, total: nc.total, formaPago: "", nota: "", motivo: "", validezDias: null };
}
