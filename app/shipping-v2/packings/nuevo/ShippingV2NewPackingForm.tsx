...
function Field({ label, help, children }: { label: string; help?: string; children: React.ReactNode }) {
  return (
    <label className="block space-y-2">
      ...
      {children}
      {help ? <span className="block text-xs text-[#A7A7A7]">{help}</span> : null}
    </label>
  );
}

const inputClass = "h-12 w-full rounded-full border border-[#3A3A36] bg-[#151515] px-4 text-sm text-[#F5F5F5] outline-none transition focus:border-[#D7FF4F]/70";
...
export function ShippingV2NewPackingForm({ proveedores }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full space-y-5 rounded-[2rem] border border-[#3A3A36] bg-[#151515] p-4 shadow-2xl shadow-black/40 sm:p-5">
      <section className="rounded-[1.75rem] border border-[#3A3A36] bg-[#1E1F1C] p-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <span className="rounded-full border border-[#D7FF4F] bg-[#D7FF4F] px-3 py-1 text-[11px] font-bold uppercase text-[#151515]">Nuevo Packing</span>
            <h2 className="mt-4 text-2xl font-semibold text-[#F5F5F5]">Crear Packing</h2>
            <p className="mt-1 text-sm text-[#A7A7A7]">Crea el grupo físico. No crea recepción, finanzas ni costos distribuidos.</p>
          </div>
          <Link href="/shipping-v2/packings" className="rounded-full border border-[#3A3A36] bg-[#252622] px-5 py-2.5 text-center text-sm font-medium text-[#F5F5F5] transition hover:border-[#D7FF4F]/60 hover:text-[#D7FF4F]">Volver</Link>
        </div>
      </section>

      {error ? <div className="rounded-[1.25rem] border border-[#FF914D]/35 bg-[#FF914D]/10 px-4 py-3 text-sm text-[#FFB07A]">{error}</div> : null}

      <section className="grid gap-4 rounded-[1.75rem] border border-[#3A3A36] bg-[#2A2A28] p-5 md:grid-cols-2">
        ...
        <label className="block space-y-2 md:col-span-2">
          <span className="text-[11px] font-semibold uppercase tracking-normal text-[#A7A7A7]">Observaciones</span>
          <textarea value={form.observaciones} onChange={(event) => update("observaciones", event.target.value)} className="min-h-28 w-full rounded-[1.25rem] border border-[#3A3A36] bg-[#151515] px-4 py-3 text-sm text-[#F5F5F5] outline-none transition focus:border-[#D7FF4F]/70" />
        </label>
      </section>

      <div className="flex justify-end">
        <button disabled={saving} className="rounded-full border border-[#D7FF4F] bg-[#D7FF4F] px-6 py-3 text-sm font-bold text-[#151515] transition hover:brightness-105 disabled:cursor-not-allowed disabled:opacity-60">{saving ? "Creando..." : "Crear Packing"}</button>
      </div>
    </form>
  );
}
