...
export async function GET(request: Request) {
  ...
  try {
    const catalogo = await fetchCatalogoServiciosGestion({ activo: getActivoFilter(status) });
...
export async function POST(request: Request) {
  ...
  try {
    const created = await createCatalogoServicio({
      nombre,
      descripcion: typeof body?.descripcion === "string" ? body.descripcion : null,
      costoSugerido: toNumber(body?.costoSugerido),
