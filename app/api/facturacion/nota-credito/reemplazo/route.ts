...
export async function GET(request: Request) {
  ...
  // listarNotasCredito no filtra por recordId, pero el módulo no expone un
  // getter individual; se busca en la primera página por el record id.
  const { notas } = await listarNotasCredito({ pageSize: 200 });
  const nc = notas.find((n) => n.recordId === recordId);
  if (!nc) {
