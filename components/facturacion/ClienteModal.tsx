...
export type ClienteResuelto = { id: string; nombre: string; cedula: string; telefono: string; correo: string; direccion: string };
...
export function ClienteModal({ modo, clienteId, onClose, onGuardado }: {
  ...
  useEffect(() => {
    ...
    fetch(`/api/facturacion/clientes/${clienteId}`).then((r) => r.json()).then((d) => {
      if (!vivo) return;
      if (d.success && d.data) { setNombre(d.data.nombre ?? ""); setCedula(d.data.cedula ?? ""); setTelefono(d.data.telefono ?? ""); setCorreo(d.data.correo ?? ""); setDireccion(d.data.direccion ?? ""); setNotas(d.data.notas ?? ""); }
  ...
  async function guardar() {
    ...
    try {
      if (modo === "crear") {
        const r = await fetch("/api/facturacion/clientes", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ nombre: nombre.trim(), cedula: cedula.trim(), telefono: telefono.trim(), correo: correo.trim(), direccion: direccion.trim() }) });
        ...
        const r = await fetch(`/api/facturacion/clientes/${clienteId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ nombre: nombre.trim(), cedula: cedula.trim(), telefono: telefono.trim(), correo: correo.trim(), direccion: direccion.trim(), notas }) });
        const d = await r.json();
        ...
        <div className="p-5">
          {cargando ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div><label className={LABEL}>Cédula / RUC</label><input value={cedula} onChange={(e) => { setCedula(e.target.value); setError(null); setExistente(null); }} className={INPUT} placeholder="Cédula / RUC / Pasaporte" /></div>
